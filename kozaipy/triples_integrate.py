...
def integrate_triple_system(ics,timemax,Nevals,
    ...
    rg_0, rg_1 = body0.gyroradius, body1.gyroradius
    ...
    time = np.linspace(0,timemax,Nevals)
    params = m0,m1,m2,radius0,radius1,rg_0,rg_1,k2_0,k2_1,tv0,tv1,\
             octupole_potential,\
             short_range_forces_conservative,short_range_forces_dissipative,solve_for_spin_vector

    if (integrator == 'scipy'):
        sol =integ.odeint(threebody_ode_vf,ics,time,\
                          args=params,\
        ...
        solver = integ.ode(ode_modified).set_integrator('dopri',nsteps=10000,atol=1e-9,rtol=1e-6, method='bdf')#,nsteps=10000,atol=1e-6,rtol=1e-10)
        solver.set_initial_value(ics, 0).set_f_params(*params)
        kk = 1
        sol = []
        sol.append(ics)
...
def ode_modified(t,y,m0,m1,m2,
    ...
    return threebody_ode_vf(y,t,m0,m1,m2,
                            radius0, radius1,
                            rg_0,rg_1,
                            k2_0, k2_1,
                            tv0, tv1,
...
def threebody_ode_vf(y,t,m0,m1,m2,
    ...
    if (extra_forces_conservative):
        ...
        if (extra_forces_dissipative):
            ...
            X0 += -1.0/norbit_in * Omega0_v /2/tf0/accelerate0 * (1 + 4.5 * ein_squared + 0.625 * ein_fourth) / one_minus_einsq_fifth
        
            Y0 += 1.0/norbit_in * Omega0_u /2/tf0/accelerate0 * (1 + 1.5 * ein_squared + 0.125 * ein_fourth) / one_minus_einsq_fifth
            ...
            X1 += -1.0/norbit_in * Omega1_v /2/tf1/accelerate1 * (1 + 4.5 * ein_squared + 0.625 * ein_fourth) / one_minus_einsq_fifth
            
            Y1 += 1.0/norbit_in * Omega1_u /2/tf1/accelerate1 * (1 + 1.5 * ein_squared + 0.125 * ein_fourth) / one_minus_einsq_fifth
